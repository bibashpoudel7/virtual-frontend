...
export async function generateCubeMapFromPanorama(
  ...
  for (const face of Object.values(CubeFace)) {
    ...
    // Convert to JPEG
    const faceBuffer = await sharp(facePixels, {
      raw: {
        width: cubeSize,
        height: cubeSize,
...
export async function generateCubeMapTiles(
  ...
  // Use higher base resolution for better quality
  const cubeSize = 4096; // Increased base resolution for each cube face
  const tiles: Array<CubeTile & { buffer: Buffer }> = [];
  
  // Generate cube faces from panorama at full resolution
  const cubeFaces = await generateCubeMapFromPanorama(panoramaBuffer, cubeSize);
  ...
  // Each level has progressively larger tiles
  const levelConfigs = [
    { resolution: 512, tileSize: 512 },    // Level 0: 1x1 tile of 512x512
    { resolution: 1024, tileSize: 1024 },  // Level 1: 1x1 tile of 1024x1024
    { resolution: 2048, tileSize: 1024 },  // Level 2: 2x2 tiles of 1024x1024
    { resolution: 4096, tileSize: 1024 },  // Level 3: 4x4 tiles of 1024x1024
